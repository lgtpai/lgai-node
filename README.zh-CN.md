...
| `LGAI_DB` | 自动探测 | 猎狗推送数据库路径（sqlite，`newcoins` 表）。连接后（只读，需 Node ≥ 22），推送价推土机趋势成为**预测最高优先级依据**（LGAI 推送 > AI 集成 > 人类情绪 > SMA），并对**全库所有推送项目**（约 900 个）做推土机扫描：市场宽度 + 强趋势榜，每 5 分钟刷新（`lgai-scan` 商品）。**交易范围自动扩展到有趋势的龙头项目**（白名单 `LGAI_MAJORS`，默认对齐 lgai_regime.MAJORS 十四主流币：BTC/ETH/BNB/XRP/SOL/UNI/ADA/DOGE/LINK/AVAX/ARB/SUI/APT/OP，上限 `MAX_DYN_SYMBOLS`）：采集/预测/验证任务与人类情绪投票面板全部跟随交易域；推送直发预测由推送价自裁定，反复失败币种自动熔断 |
