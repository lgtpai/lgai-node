...
| `LGAI_DB` | 自动探测 | 猎狗推送数据库路径（sqlite，`newcoins` 表）。连接后（只读，需 Node ≥ 22），推送价推土机趋势成为**预测最高优先级依据**（LGAI 推送 > AI 集成 > 人类情绪 > SMA），并对**全库所有推送项目**（约 900 个）做推土机扫描：市场宽度 + 强趋势榜，每 5 分钟刷新（`lgai-scan` 商品）。**交易范围自动扩展到全部趋势推送项目**（`MAX_DYN_SYMBOLS`，默认 20，按趋势强度排序）：采集/预测/验证任务全覆盖；未上交易所的项目走推送价直发预测 + 协调端推送价自裁定，反复失败的币种自动熔断 |
